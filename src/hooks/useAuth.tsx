  ...
  async function signIn() {
    try {
      ...
      if (authResponse.type === 'success' && authResponse.params.error !== 'access_denied') {
        ...
        const userRespose = await api.get('/users');

        setUser({
          id: userRespose.data.data[0].id,
          display_name: userRespose.data.data[0].display_name,
          email: userRespose.data.data[0].email,
          profile_image_url: userRespose.data.data[0].profile_image_url,
        });
